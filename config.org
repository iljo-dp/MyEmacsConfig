#+TITLE: Iljo's GNU emacs config(based on DT's serie)
...
- [[#emacs-qol-tweaks][EMACS QOL TWEAKS]]
...
* ALL THE ICONS
Dont' worship them. Maar gebruik ze wijs haha.
Dit is een icon set voor dingen zoals dashboard, dired, ibuffer en nog andere emacs stuff
#+begin_src emacs-lisp
...
#+end_src
* BUFFER-MOVE
Bewegen tussen verschillende buffers in emacs(tmux??)
...
#+end_src
* IVY (COUNSEL)
+ Ivy, het generieke completion framework van Emacs
+ Counsel, een verzameling van Ivy-enhanced versies van common Emacs
+ Ivy-rich laat ons descripties aan commands toevoegen in de M-x.
#+begin_src emacs-lisp
  (use-package counsel
    :after ivy
    :config (counsel-mode))
...
** LSP MODE
This is to make emacs and lsp and actually code with it
#+begin_src emacs-lisp
...
* RAINBOW MODE
Zorgt dat kleuren worden weergegeven als de kleur volgens hex. (#ffffff)
#+begin_src emacs-lisp
...
** Eshell
Shell geschreven in emacs-lisp
#+begin_src emacs-lisp
...
** Vterm
Terminal-emulator die in emacs zelf werkt. 
#+begin_src emacs-lisp
(use-package vterm
:config
...
#+end_src
** Vterm-Toggle
Toggled tussen onze buffer en de Vterm
#+begin_src emacs-lisp
...
* SUDO EDIT
-*sudo-edit*- Geeft ons de kracht om files te openen met sudo in emacs, of een file opnieuw te openen in een sudo buffer
#+begin_src emacs-lisp
